...
if __name__=='__main__':
    ...
    for i in tqdm(range(len(files)), file=sys.stdout):
        ...
        # NOTE: due to the kind of activities done in the pipeline, it is better to have a Process, which has a dedicated private memory, than a Thread
        works.append(Process(target=run, args=(files[i], img_type, atlas_file)))
        works[-1].start()            
